...
files_cc = np.array(glob.glob(os.path.join('..', 'compare_res', 'ccsn_*')))
n_file = len(files_cc)
...
n_cc = np.zeros((n_file, mlev, nlev2))
m_cc = np.zeros((n_file, mlev))
z_cc = np.zeros((n_file, nlev2))
t_cc = np.zeros((n_file, nlev2))
r_cc = np.zeros((n_file, nlev2))

i_file = 0
for f in files_cc:
    data = np.loadtxt(f)
    print os.path.basename(f)
    if len(data[:,1]) == nlev*mlev:
    ...
    if len(data[:,1]) == nlev2*mlev:
        for i in range(0, len(data[:,1])):
            n_cc[i_file, i/nlev2, i % nlev2] = data[i, 0]
            m_cc[i_file, i/nlev2] = data[i, 1]
            z_cc[i_file, i % nlev2] = data[i, 2]
            t_cc[i_file, i % nlev2] = data[i, 3]*yr
        temp = cumtrapz(dr(z_cc[i_file, :]), x=z_cc[i_file, :])
        z_0 = trapz(dr(np.linspace(0, z_cc[i_file, 0], 1000)), x=np.linspace(0, z_cc[i_file, 0], 1000))
        r_cc[i_file,:] = np.hstack([z_0, temp+z_0])
...
files_pi = glob.glob(os.path.join('..', 'compare_res', 'pisn_*'))
files_pi.sort()
i_file = 0

n_pi = np.zeros((n_file, mlev, nlev2))
m_pi = np.zeros((n_file, mlev))
z_pi = np.zeros((n_file, nlev2))
t_pi = np.zeros((n_file, nlev2))
r_pi = np.zeros((n_file, nlev2))
...
for f in files_pi:
    ...
    if len(data[:,1]) == nlev2*mlev:
        for i in range(0, len(data[:,1])):
            n_pi[i_file, i/nlev2, i % nlev2] = data[i, 0]
            m_pi[i_file, i/nlev2] = data[i, 1]
            z_pi[i_file, i % nlev2] = data[i, 2]
            t_pi[i_file, i % nlev2] = data[i, 3]*yr
        temp = cumtrapz(dr(z_pi[i_file, :]), x=z_pi[i_file, :])
        z_0 = trapz(dr(np.linspace(0, z_pi[i_file, 0], 1000)), x=np.linspace(0, z_pi[i_file, 0], 1000))
        r_pi[i_file,:] = np.hstack([z_0, temp+z_0])
    i_file = i_file+1
...
plt.savefig('../plots/res/test/CCSN_angle_comp.jpg', bbox_inches='tight')
plt.show()
plt.clf()

# redshift plots
print 'plotting as function of redshift'
rate_z_pi = np.zeros((n_file, nlev2-1))
for j_file in  range(0, n_file):
...
plt.savefig('../plots/res/test/PISN_angle_comp.jpg', bbox_inches='tight')
plt.show()
plt.clf()
