...
files_cc = glob.glob(os.path.join('..', 'work', 'output_vanilla', 'sn_rates', 'ccsn*'))
files_cc.sort()
i_file = 0
n_file = len(files_cc)
...
files_pi = glob.glob(os.path.join('..', 'work', 'output_vanilla', 'sn_rates', 'pisn*'))
files_pi.sort()
i_file = 0
n_file = len(files_pi)
...
plt.title(r'Core Collapse Supernova Rates in $1000 Mpc^3$')
plt.savefig('../plots/work/CCSN_z.jpg', bbox_inches='tight')
plt.show()
plt.clf()
...
# plt.yscale('log')
plt.legend(bbox_to_anchor=(1.6, 1))
plt.title(r'Pair Instability Supernova Rates in $1000 Mpc^3$')
plt.savefig('../plots/work/PISN_z.jpg', bbox_inches='tight')
plt.show()
plt.clf()
